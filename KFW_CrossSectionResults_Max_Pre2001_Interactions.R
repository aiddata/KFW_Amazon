...
stargazer(OutputEarly3$standardized, OutputEarly3_1$standardized, 
          ...
          covariate.labels=c("Treatment", "PretrendMax Interaction", "MaxL1995 Interaction", 
          ...
          dep.var.labels=c("Max NDVI 1995-2001"),
...
##INTERACTION TERMS
#Interaction pretrend NDVI & treatment
Data_Late@data["pretrend_Trt_1"] <- ((Data_Late@data$pre_trend_NDVI_max)*(Data_Late@data$TrtBin))

#Interaction MeanL1995 & treatment
Data_Late@data["MaxL1995_Trt_1"] <- ((Data_Late@data$MaxL_1995)*(Data_Late@data$TrtBin))
...
analyticModelLate_1 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + pretrend_Trt_1"

analyticModelLate_2 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + MaxL1995_Trt_1"

analyticModelLate_3 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + area_Trt_1"

analyticModelLate_4 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + Pop_Trt_1"

analyticModelLate_5 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + MeanT_Trt_1"

analyticModelLate_6 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + postT_Trt_1"

analyticModelLate_7 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + MeanP_Trt_1"

analyticModelLate_8 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + posttrendP_Trt_1"

analyticModelLate_9 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + Slope_Trt_1"

analyticModelLate_10 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + Elevation_Trt_1"

analyticModelLate_11 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
MeanP_B + post_trend_precip + Slope + Elevation + Riv_Dist + Road_dist + factor(PSM_match_ID) + RivDist_Trt_1"

analyticModelLate_12 <- "NDVILevelChange_01_10 ~ TrtBin + pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B + post_trend_temp + 
...
stargazer(OutputLate$standardized,OutputLate_1$standardized,OutputLate_2$standardized,
          ...
          keep=c("TrtBin", "pretrend_Trt_1","MaxL1995_Trt_1", "area_Trt_1","Pop_Trt_1", "MeanT_Trt_1","postT_Trt_1","MeanP_Trt_1",
                 "posttrendP_Trt_1", "Slope_Trt_1","Elevation_Trt_1","RivDist_Trt_1","RoadDist_Trt_1"),
          covariate.labels=c("Treatment", "PretrendMax Interaction", "MaxL1995 Interaction", 
                             "Area Interaction", "Pop Interaction,", "MeanT Interaction", "PostT Interaction", "MeanP INteraction", 
                             "PosttrendP Interaction", "Slope Interaction", "Elevation Interaction", "RivDist Int", "RoadDist Int"),
          dep.var.labels=c("Max NDVI 2001-2010"),
          title="Regression Results", type="html", omit.stat=c("f","ser"), align=TRUE)
