...
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="TrtBin")] <- "Treatment"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="enforce_to")] <- "Enforcement_Years"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="pre_trend_NDVI_max")] <- "NDVI_PreTrends"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="MaxL_1995")] <- "NDVI_Baseline"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="Riv_Dist")] <- "River_Distance"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="Road_dist")] <- "Road_Distance"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="terrai_are")] <- "Area"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="Pop_1990")] <- "Population"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="MeanT_1995")] <- "Mean_Temp"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="MeanP_1995")] <- "Mean_Precip"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="post_trend_temp_mean")] <- "Temp_Trends"
colnames(Data_Ever4@data)[(colnames(Data_Ever4@data)=="post_trend_precip_mean")] <- "Precip_Trends"

analyticModelEver4 <- "NDVILevelChange_95_10 ~ Treatment + Enforcement_Years + NDVI_PreTrends + NDVI_Baseline + Area + Population + Mean_Temp + Temp_Trends +
...
texreg::plotreg(OutputEver4$standardized, omit.coef="(Intercept)|(factor)", 
                custom.model.names=c("Cross-Section Results, Max NDVI, 1995-2010"), 
